...
function gameLoop(){
  //game over by losing all your lives 
  ...
  //winning the game by killing the invaders
  if(aliveInvaders == 0){
    gameRunning=false;
...
function moveInvaderBullets(){
  for(let i = 0 ; i < invaderBulletsArray.length; i++){
    ...
    // checking to see if the games over by getting hit
    if(
      iB.x > tankX &&
    ...
    for (let i = 0; i < armyRows; i++) {
      for(let j = 0; j < armyColumns; j++){
          ...
          )
          {
            ...
            // making the speed go faster
            if((aliveInvaders)%armyColumns==0 && armySpeed > 25){
